...
// MANEJO DE LA PORTADA Y AUDIO (SOLO POR CLIC EN EL ICONO)
mainIcon.addEventListener('click', (e) => {
    // Esto evita que el clic haga cosas raras en otros elementos
    e.stopPropagation();

    // 1. Intentar reproducir audio principal
    if (mainAudio) {
        mainAudio.play()
            .then(() => {
                musicDisk.classList.add('playing');
                // IMPORTANTE: Hacemos visible el disco de música al entrar
                musicDisk.classList.add('visible');
            })
            .catch(err => {
                console.log("Audio bloqueado por el navegador, se activará al interactuar:", err);
                // Si el navegador bloquea el auto-play, igual mostramos el botón
                musicDisk.classList.add('visible');
            });
    }
    
    // 2. Sonido de clic opcional
    if (clickSound) clickSound.play();
    
    // 3. Efecto de explosión de la portada
    cover.classList.add('explode');
    
    // 4. Esperar a que termine la animación para ocultar y mostrar contenido
    setTimeout(() => {
